// !!! Includes patches from Socialcast.  See "Socialcast-patched" in this file !!!
...
(function($) {
    ...
    ajax: function(origSettings) {
      ...
      $.each(mockHandlers, function(k, v) {
          ...
          if ( m ) {
            ...
            if ( m.data && s.data ) {
              ...
              (function ident(mock, live) {
                ...
                } else if (jQuery.isEmptyObject(live)) {
                  // Fall-through means they are an exact match of empty objects.
                  identical = true;
                  return true;
                }
            ...
            // Socialcast-patched to add case-insensitive matches to mockjax type
        ...
        if ( m ) {
          ...
          // Socialcast-patched to expose data sent with request in mock object
          mockHandlers[k].data = s.data;
          mockHandlers[k].cache = s.cache;
          mockHandlers[k].timeout = s.timeout;
          mockHandlers[k].global = s.global;
          mockHandlers[k].requestHeaders = {};
            ...
            xhr: function() {
              ...
              return {
                ...
                send: function() {
                  ...
                  // Socialcast-patched to expose if request was sent (from mock object)
                ...
                setRequestHeader: function() {
                  // Socialcast-patched to expose headers that were sent (from mock object)
                  if (arguments && arguments.length == 2){
                    mockHandlers[k].requestHeaders[arguments[0]] = arguments[1];
                  }
                },
          ...
          mockHandlers[k].abort = mock.abort;
          return false;
        }
      });
      // We don't have a mock request, return false ( we should always have a mock)
      if ( !mock ) {
  ...
  $.mockjaxClear = function(i) {
    if ( arguments.length == 1 ) {
      ...
      // Abort in-progress XHRs.
      // To prevent callbacks from firing in subsequent tests
      $.each(mockHandlers, function(){
        var request = this;
        if (request.fired && !request.returned && request.abort){
          try {
            request.abort();
          }catch(e){};
        }
